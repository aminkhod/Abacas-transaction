...
def getClientRef(c, clientlist):
    num = str(clientlist.loc[c,'SR. NO.']) + '.'
    num = num + clientlist.loc[c,'LIST OF COMPANY'][:4]
#     for char in clientlist.loc[c,'LIST OF COMPANY']:
#         if char != '.':
#             num = num + char
#         else:
#             break
    return num
def exactMatch(candid,clientlist):
    candid = ''.join(e for e in candid if e.isalnum())
    candidRow = []
    c = 0
    num = ''
    for client in clientlist['LIST OF COMPANY']:
        client = ''.join(e for e in client if e.isalnum())
        if client == candid:
            candidRow = list(clientlist.iloc[c])
            num = getClientRef(c, clientlist)
        c +=1
...
#     num , vec = bestmatch(client,clientlist)
    num, vec = exactMatch(client,clientlist)
    numlist.append(num)
    veclist.append(vec)
    if vec != []:
...
def getRandomKProduct(amount,iD, datalist):
    ...
    for proAmount in tradedataframe['dollar']:
        proAmount = np.round(proAmount, decimals=2)
        if amount > proAmount:
            codeVector.append(True)
        else:
            codeVector.append(False)
...
for i in range(len(track['date'])):
    ...
    copyamount = amount = np.round(float(track.loc[i,'amount']), decimals=2)
...
#                 print((amount - (unitp[-1] * tempqty))/float(RandomKProduct.loc[j + 1,'dollar']))
            copyamount = np.round(copyamount - (unitp[-1] * tempqty), decimals=2)
            qty.append(tempqty)
            total.append(np.round((unitp[-1] * qty[-1]), decimals=2))
            tot += np.round((unitp[-1] * qty[-1]), decimals=2)
...
for city in cityOfImporter:
    ...
    dischargeWithoutDischarge.append(str(city))
    if city == '':
...
forPO['country of manufacture'] = countryOfManufacture
forPO['country of destination'] = countryOfDestination

forPO['date of PO'] = datePO
